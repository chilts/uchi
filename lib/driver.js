...
Driver.prototype.removeMulti = function(keys, callback) {
    var self = this;

    async.whilst(
        function() { return keys.length; },
        function(callback) {
            var key = keys.shift();
            self.remove(key, callback);
        },
        function(err) {
            // everything has been removed (or something failed)
            callback(err);
        }
    );
};

// clears all the keys (in this namespace)
Driver.prototype.clear = function(callback) {
    var self = this;
    self.getKeys(function(err, keys) {
        self.removeMulti(keys, function(err) {
            callback(err);
        });
    });
};
...
// retrieves the desired key from the cache, but doesn't no checking on it except a transformation into a CacheObject
